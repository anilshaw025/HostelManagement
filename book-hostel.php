...
$feespm=$_REQUEST['fpm'];
$foodstatus=$_REQUEST['foodstatus'];
$stayfrom=$_REQUEST['stayf'];
...
$emcntno=$_REQUEST['econtact'];
$gurname=$_REQUEST['gname'];
$gurrelation=$_REQUEST['grelation'];
$gurcntno=$_REQUEST['gcontact'];
$caddress=$_REQUEST['address'];
$ccity=$_REQUEST['city'];
$cstate=$_REQUEST['state'];
$cpincode=$_REQUEST['pincode'];
...
$target = "admin/img/".basename($image);

$ret=mysqli_query($mysqli, "select email from registration where email='$email' ");
    $result=mysqli_fetch_array($ret);
    if($result>0)
        {
           echo "<script>alert('You had already booked a room.');</script>";
        }
else{
$query="insert into  registration(roomno,seater,feespm,foodstatus,stayfrom,duration,course,firstName,middleName,lastName,gender,contactno,email,aadhar,egycontactno,guardianName,guardianRelation,guardianContactno,corresAddress,corresCIty,corresState,corresPincode,pmntAddress,pmntCity,pmnatetState,pmntPincode,image) value('$roomno','$seater','$feespm','$foodstatus','$stayfrom','$duration','$course','$fname','$mname','$lname','$gender','$contactno','$email','$aadhar','$emcntno','$gurname','$gurrelation','$gurcntno','$caddress','$ccity','$cstate','$cpincode','$paddress','$pcity','$pstate','$ppincode','$image')";

$rc=mysqli_query($mysqli,$query);

if (move_uploaded_file($_FILES['image']['tmp_name'], $target) && $rc>0)
            {
...
//     if($result>0)
//         {
//            echo "<script>alert('You already booked a room.');</script>";
//         }
...
// // $rc=$stmt->bind_param('iiiisisssssissississsisssii',$roomno,$seater,$feespm,$foodstatus,$stayfrom,$duration,$course,$fname,$mname,$lname,$gender,$contactno,$email,$aadhar,$emcntno,$gurname,$gurrelation,$gurcntno,$caddress,$ccity,$cstate,$cpincode,$paddress,$pcity,$pstate,$ppincode,$imageContent);
// // $stmt->execute();
// $rc=mysqli_query($mysqli,$query);
			...
			<div class="container-fluid">
				...
				<div class="row">
					<div class="col-md-12">
						...
						<h2 class="page-title" style="margin-top:4%">Registration </h2>
...
$uid=$_SESSION['login'];
				...
				if($rs)
				{ ?>
			<h3 style="color: red" align="left">one room is already booked by you</h3>
				<?php }
				else{
							echo "";
							}			
...
<div class="col-sm-8">
<input type="text" name="aadhar" id="aadhar"  class="form-control" required="required" >
</div>
</div>
